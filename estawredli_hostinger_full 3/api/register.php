...
try {
    ...
    // إدخال المستخدم في قاعدة البيانات بوضع المعلق (pending) لكي يوافق عليه الأدمن
    $insert_stmt = $pdo->prepare("INSERT INTO users (name, email, password, phone, role, status) VALUES (?, ?, ?, ?, 'customer', 'pending')");
    
    if ($insert_stmt->execute([$name, $email, $hashed_password, $phone])) {
        
        // إرسال إيميل ترحيبي للمستخدم بأن حسابه قيد المراجعة
        $user_subject = "مرحباً بك في متجر استوردلي - حسابك قيد المراجعة";
        $user_message = "مرحباً " . $name . "،

شكراً لتسجيلك في متجر استوردلي.
حسابك الآن قيد المراجعة من قبل الإدارة. سوف يتم التواصل معك قريباً لتفعيل حسابك وإظهار الأسعار.

مع تحيات فريق استوردلي.";
        $headers = "From: noreply@" . $_SERVER['HTTP_HOST'];
        mail($email, $user_subject, $user_message, $headers);
        
        // إرسال إيميل للأدمن بطلب التفعيل
...
يرجى الدخول للوحة التحكم لتفعيل الحساب.";
        ...
        echo json_encode([
            'success' => true, 
            'message' => 'تم إنشاء الحساب بنجاح وهو قيد المراجعة حالياً. سيتم تفعيل حسابك من قبل الإدارة قريباً! 🎉'
        ]);
    } else {
