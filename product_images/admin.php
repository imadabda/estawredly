...
async function saveAdminProducts() {
  ...
  try {
      ...
      if (!data.success) {
          ...
          alert("خطأ في الحفظ على السيرفر: " + (data.message || 'فشل الحفظ'));
          return false;
      } else {
      ...
      alert("⚠️ تعذر الاتصال بالسيرفر لحفظ المنتجات. يرجى التأكد من الاتصال. التفاصيل: " + err);
      return false;
  }
}
...
async function saveProduct() {
  ...
  if (saveBtn) {
    ...
    saveBtn.innerHTML = '⏳ جاري الحفظ على السيرفر...';
  }
  ...
  const success = await saveAdminProducts();
  if (saveBtn) {
    saveBtn.disabled = false;
    saveBtn.innerHTML = saveBtn.dataset.origText || '💾 حفظ المنتج بالنظام';
  }

  if (success) {
    showToast(editingId ? '✅ تم تحديث المنتج بنجاح!' : '✅ تم إضافة المنتج للمتجر بنجاح!');
    closeModal();
    renderProducts();
    updateStats();
  }
...
async function deleteProduct(id) {
  ...
  const oldList = [...adminProducts];
  adminProducts = adminProducts.filter(p => String(p.id) !== String(id));
  renderProducts();
  showToast('⏳ جاري الحذف من السيرفر...');
  const success = await saveAdminProducts();
  if (success) {
    updateStats();
    showToast('🗑️ تم حذف المنتج بنجاح من السيرفر');
  } else {
    adminProducts = oldList;
    renderProducts();
    showToast('❌ فشل الحذف على السيرفر', 'warn');
  }
}
...
const adminHeroBanner = {
    ...
    init() {
        try {
            const cached = localStorage.getItem('estawredly_hero_banner');
            if (cached) {
                this.data = Object.assign(this.data, JSON.parse(cached));
                this.populate();
            }
        } catch(e){}
        this.load();
    ...
    async save() {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                try { localStorage.setItem('estawredly_hero_banner', JSON.stringify(this.data)); } catch(e){}
